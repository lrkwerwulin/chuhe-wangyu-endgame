...
[![Verified puzzles](https://img.shields.io/badge/forced--win%20puzzles-12-a9463d.svg)](./lib/puzzles.ts)
...
- **12 个可玩强胜残局**：8 枚 M2 与 4 枚 M5，覆盖 2 VS 4、3 VS 4、3 VS 5；M5 的最短主线为 9 ply。
- **连续完成胜法**：玩家落子后，系统执行主证明线中的最长抵抗，直至 M2 或 M5 终局；界面逐题公开唯一决策范围。
- **逐层预测漏斗**：把每条合法首着在 H1–H7 或 H1–H9 分别标成“已证胜 / 已证负 / 未决”，并记录立即回应数与首次得证深度。
...
| 根节点资格 | 在题目证明窗口内恰好一条首着可证明强胜 |
| 最短杀程 | M2 必须首次在 H3 得证；M5 必须在 H1–H8 无强杀、首次在 H9 得证 |
| 前段唯一 | `uniqueWinnerTurns` 指定沿全部防守分支必须保持唯一的前几次胜方决策 |
| 后段上限 | `maxWinningMoves` 限制其余胜方节点的等价胜着数；“五着孤径”五次决策全部唯一 |
| 复证深度 | M2 使用 7 ply 排查替代胜着，M5 使用完整 9 ply 证明最短终局 |

这里的“唯一”是公开有限窗口内的严格结论，不是无限深度表库声明。未入选走法可能在更深层仍有胜机，但不能在题目窗口内达到同样的最短强胜；这一边界会在界面、数据和验证脚本中保持一致。
...
4 枚 M5 另有 49 条合法首着：H1–H8 全部保持未决，直到 H9 才同时出现 4 条已证胜、45 条未决。这正是“最短 M5”的可执行证据，而不是只保存一条看起来能赢的 9-ply 线路。

全库合计 232 条合法首着、4,590 条立即回应边，单着分支数为 1–54，平均 19.78。逐层分类与刚性复证合计访问 3,835,318 个节点，完成 539,602 次 alpha-beta 剪枝并命中置换表 393,245 次。搜索成本会随法排序和设备变化，三态漏斗、杀程与唯一性结论则由回归测试锁定。
...
这类 RL 式排序可以节省搜索顺序成本，却不能安全删除守方分支。最终结论始终由无静态估值的 mate-only 搜索给出：H1 到目标深度逐层加深，任何节点预算超限都只算“未完成”。本轮还精确发现了 M6（11 ply）候选，并把根胜着从 4 条抛光到 2 条；因为未达到唯一首着，未收入正式题库。

当前 12 题共覆盖 232 个合法首着，每题在自己的 H7/H9 窗口内恰好 1 条强胜首着。本轮长杀阶段从 M1 逐层构造到 M5，又对单马 75 格、守方兵 83 格等邻域执行双预算扫描；正式保留 4 枚 M5，其中“六十六防线”覆盖 66 条深层防守边，“五着孤径”的五次胜方决策全部唯一。
...
| `pnpm measure:horizons` | 按每题证明深度汇总 H1–H7/H9 三态漏斗与搜索成本 |
...
| `pnpm search:long-mates -- --target-moves=5 --seed=17` | 奖励驱动地逐层构造并严格证明 M5–M10 候选 |
| `pnpm scan:long-mates -- --source=nine-ply-a-file-knight-net --target-moves=6` | 双预算扫描单子邻域并寻找更长最短杀 |
...
2. `expectedWinningMoveKeys` 与题目证明窗口的求解结果完全一致；
3. `expectedHorizonCounts` 与 H1–H7 或 H1–H9 的胜/负/未决漏斗完全一致；
