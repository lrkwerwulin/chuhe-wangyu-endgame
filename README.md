...
- **浏览器内重新证明**：可在当前设备重新搜索七层全分支树，查看决策点、防守边、节点、剪枝和置换表命中。
...
当前 8 题共覆盖 183 个合法首着，七层窗口内每题恰好 1 条强胜首着。一次完整的根着分类与刚性复证共访问约 114 万个节点。本轮发现阶段另外检查了 8,000 余个合法随机布局、1,824 个单子变异布局以及多批逆向回溯候选；节点数可能随着法排序实现变化。
...
3. 执子方在窗口内可以强制终局，且根节点恰好一条胜着；
4. 沿守方全部合法分支到达的第二次胜方决策仍恰好一条胜着；
5. 存档主变化合法并以对方将死或困毙负结束；
6. `pnpm verify:puzzles`、`pnpm audit:engine` 与 `pnpm build` 全部通过；
7. 题目不是只靠无关添子制造的重复构型。
