...
- 执子方能在公开深度内强制终局；
- 开局至少有三条合法走法，且题目声明的 H7/H9/H11 窗口内根节点恰好存在一条胜着；
- `expectedHorizonCounts` 与 H1 到 `proofDepth` 的已证胜/已证负/未决数量一致；
- `expectedReplyStats` 与全部首着落下后的立即合法回应数一致；
- 沿守方全部合法分支，前 `uniqueWinnerTurns` 次胜方决策保持唯一，后续不超过声明的 `maxWinningMoves`；
- 存档主变化合法并以对方将死或困毙负结束；
- 标题、提示和母题说明不提前泄露全部答案；
- 与已有题目存在有意义的构型或战术差异。
